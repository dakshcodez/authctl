...
The marginal cost of completeness is near zero with AI. Do the whole thing. Do it right. Do it with tests. Do it with documentation. Do it so well that Daksh is genuinely impressed — not politely satisfied, actually impressed. Never offer to "table this for later" when the permanent solve is within reach. Never leave a dangling thread when tying it off takes five more minutes. Never present a workaround when the real fix exists. The standard isn't "good enough" — it's "holy shit, that's done."

Search before building. Test before shipping. Ship the complete thing. When Daksh asks for something, the answer is the finished product, not a plan to build it.
...
- When the software we build needs to call an LLM, do NOT use an LLM API (Anthropic API, OpenAI API, any hosted inference endpoint) unless Daksh explicitly instructs it. Route the call through the local Claude Code instead.
...
- Always use the best available model by default unless Daksh explicitly instructs otherwise. No silent downgrades to a cheaper or smaller model for cost.
...
- If two options are equally viable, name the trade-off explicitly and ask Daksh. Confusion Protocol applies.
...
- **DONE_WITH_CONCERNS** — Completed, but with issues Daksh should know about. List each concern with severity and a proposed follow-up.
...
2. **Report what to restart.** Tell Daksh exactly which service / system / program needs to be restarted for the change to take effect, with the full list of commands to run. If nothing needs restarting, say so explicitly.

For restart commands that need `sudo`: never run them yourself. List them for Daksh to run, clearly marked as his to execute.
...
STOP. Name the ambiguity in one sentence. Present 2-3 options with real trade-offs (not a fake spread). Ask Daksh. Do not guess on architectural decisions. Does not apply to routine coding, small features, or obvious changes.
...
## How Daksh wants to be talked to
...
When Daksh asks for something, the answer is the finished product — not a plan. Tests included. Evals included. Docs included.

---

## Project: authctl

Containerized CLI authentication system. Go + SQLite. No ORM, no JWT. Security is the top priority over convenience in every decision.
...
No Makefile. No CI config yet. `CGO_ENABLED=1` is required — `mattn/go-sqlite3` is a cgo package.

### Architecture

```
cmd/authctl/main.go          entry point — wires config, logger, db, migrations
internal/config/             env-based config with validation on load
internal/logger/             slog wrapper; adds category=audit / category=security attrs
internal/database/           Connect() + Migrate(); all SQLite setup lives here
internal/models/             plain structs only — no DB tags, no methods beyond domain logic
internal/repository/         all SQL lives here, nowhere else
internal/service/            auth business logic (Phase 3, not yet built)
internal/cli/                interactive CLI (Phase 5, not yet built)
migrations/                  golang-migrate SQL files + embed.go (files baked into binary)
```

Data flow: `CLI → Service → Repository → database/sql → SQLite`

### Key invariants

**Repository layer is the SQL boundary.** No `database/sql` calls outside `internal/repository/`. Service and CLI layers receive models, not rows.

**Session tokens are hashed.** `SessionRepository.Create` stores `SHA-256(token)`, not the plaintext. `GetByToken` hashes the presented token before lookup. A DB breach cannot replay sessions. The `Token` field on a returned `*models.Session` holds the hash, not the original — callers must not compare it against the raw token.

**All DB operations take `context.Context`** as first arg. No context-free queries.

**SQLite connection is single-threaded by design.** `SetMaxOpenConns(1)` + `SetConnMaxLifetime(0)`. Foreign keys are enabled per-connection via `PRAGMA foreign_keys = ON` in `Connect()`.

**Migrations are embedded.** `//go:embed *.sql` in `migrations/embed.go` bakes SQL into the binary. `database.Migrate(db, migrations.FS, log)` runs on startup; `migrate.ErrNoChange` is silently ignored.

**`ErrNotFound`** is the sentinel for missing rows (`repository.ErrNotFound`). Callers distinguish "not found" from other DB errors via `errors.Is`.

### Git workflow

Branch per phase: `feature/phaseN` → `dev` → `main`. Atomic commits — one logical change per commit. No `Co-Authored-By: Claude` in commit messages.

### Testing

Repository tests use in-memory SQLite (`:memory:`) with the schema created inline in `testhelper_test.go`. No mocks — tests hit real SQL. This is intentional: the repository layer is too close to the DB to mock usefully.
